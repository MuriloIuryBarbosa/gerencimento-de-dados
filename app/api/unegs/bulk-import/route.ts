  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        // Verificar se UNEG já existe (por código)
        if (row.codigo) {
          const existingUneg = await prisma.uNEG.findUnique({
            where: { codigo: row.codigo.toString() }
          });

          if (existingUneg) {
            results.duplicates.push(`Linha ${rowNumber}: UNEG com código '${row.codigo}' já existe`);
            continue;
          }
        }
        ...
        mappings.forEach(mapping => {
          ...
          if (value !== undefined && value !== null && value !== '') {
            ...
            switch (mapping.dbField) {
              case 'ativo':
                unegData[mapping.dbField] = ['true', '1', 'sim', 'yes', 'ativo'].includes(stringValue.toLowerCase());
                break;
              default:
                // Preservar caracteres especiais e remover apenas espaços extras
                unegData[mapping.dbField] = stringValue.trim();
            }
          }
        });

        // Definir valores padrão
        if (unegData.ativo === undefined) unegData.ativo = true;

        // Criar UNEG
        await prisma.uNEG.create({
          data: unegData
        });
    ...
    if (results.imported > 0 && results.errors.length === 0) {
      results.message = `${results.imported} UNEGs importadas com sucesso!`;
    } else if (results.imported > 0 && results.errors.length > 0) {
      results.message = `${results.imported} UNEGs importadas, ${results.errors.length} erros encontrados`;
    } else if (results.errors.length > 0) {
      results.message = `Falha na importação: ${results.errors.length} erros encontrados`;
      results.success = false;
    } else {
      results.message = 'Nenhuma UNEG foi importada';
      results.success = false;
    }

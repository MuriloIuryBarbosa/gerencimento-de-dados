  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        // Verificar se família já existe (por código)
        if (row.codigo) {
        ...
        mappings.forEach(mapping => {
          ...
          if (value !== undefined && value !== null && value !== '') {
            ...
            switch (mapping.dbField) {
              case 'codigo':
                familiaData[mapping.dbField] = parseInt(stringValue.replace(/[^\d]/g, ''));
                break;
              case 'ativo':
                familiaData[mapping.dbField] = ['true', '1', 'sim', 'yes', 'ativo'].includes(stringValue.toLowerCase());
                break;
              default:
                // Preservar caracteres especiais e remover apenas espaços extras
                familiaData[mapping.dbField] = stringValue.trim();
            }
          }
        });

        // Definir valores padrão
        if (familiaData.ativo === undefined) familiaData.ativo = true;
    ...
    if (results.imported > 0 && results.errors.length === 0) {
      results.message = `${results.imported} famílias importadas com sucesso!`;
    } else if (results.imported > 0 && results.errors.length > 0) {
      results.message = `${results.imported} famílias importadas, ${results.errors.length} erros encontrados`;
